...
public class StockDao_Impl(
  ...
  init {
    ...
    this.__insertAdapterOfStockEntity = object : EntityInsertAdapter<StockEntity>() {
      protected override fun createQuery(): String = "INSERT OR REPLACE INTO `stocks` (`symbol`,`name`,`nameTH`,`businessDescription`,`sector`,`industry`,`cost`,`quantity`,`tradePurpose`,`buyFees`,`dividendPerShare`,`lastPrice`,`change`,`percentChange`,`pe`,`pbv`,`roe`,`eps`,`netProfit`,`equity`,`debtToEquity`,`dividendYield`,`dividendDate`,`rsi`,`macdHist`,`signalType`,`signalReason`,`signalDescription`,`lastUpdated`,`stopLoss`,`playbookNote`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
  ...
  public override suspend fun insertStocks(stocks: List<StockEntity>): Unit = performSuspending(__db, false, true) { _connection ->
    __insertAdapterOfStockEntity.insert(_connection, stocks)
  }
    ...
    return createFlow(__db, false, arrayOf("stocks")) { _connection ->
      ...
      try {
        ...
        while (_stmt.step()) {
          ...
          _item = StockEntity(_tmpSymbol,_tmpName,_tmpNameTH,_tmpBusinessDescription,_tmpSector,_tmpIndustry,_tmpCost,_tmpQuantity,_tmpTradePurpose,_tmpBuyFees,_tmpDividendPerShare,_tmpLastPrice,_tmpChange,_tmpPercentChange,_tmpPe,_tmpPbv,_tmpRoe,_tmpEps,_tmpNetProfit,_tmpEquity,_tmpDebtToEquity,_tmpDividendYield,_tmpDividendDate,_tmpRsi,_tmpMacdHist,_tmpSignalType,_tmpSignalReason,_tmpSignalDescription,_tmpLastUpdated,_tmpStopLoss,_tmpPlaybookNote)
          _result.add(_item)
        }
        _result
      } finally {
  ...
  public override suspend fun getAllStocksSync(): List<StockEntity> {
    val _sql: String = "SELECT * FROM stocks"
    ...
    return performSuspending(__db, true, false) { _connection ->
      ...
      try {
        ...
        if (_stmt.step()) {
          ...
          val _tmpPlaybookNote: String
          _tmpPlaybookNote = _stmt.getText(_columnIndexOfPlaybookNote)
          _result = StockEntity(_tmpSymbol,_tmpName,_tmpNameTH,_tmpBusinessDescription,_tmpSector,_tmpIndustry,_tmpCost,_tmpQuantity,_tmpTradePurpose,_tmpBuyFees,_tmpDividendPerShare,_tmpLastPrice,_tmpChange,_tmpPercentChange,_tmpPe,_tmpPbv,_tmpRoe,_tmpEps,_tmpNetProfit,_tmpEquity,_tmpDebtToEquity,_tmpDividendYield,_tmpDividendDate,_tmpRsi,_tmpMacdHist,_tmpSignalType,_tmpSignalReason,_tmpSignalDescription,_tmpLastUpdated,_tmpStopLoss,_tmpPlaybookNote)
        } else {
          _result = null
        }
        _result
  ...
  public override suspend fun deleteWatchlistStocks() {
    val _sql: String = "DELETE FROM stocks WHERE quantity = 0 AND cost = 0.0"
